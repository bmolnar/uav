...
test_membuf()
{
    ...
    membuf_init(&mbuf, data, sizeof(data));

    xs = xstream_membuf_open("", &mbuf);
    if (xs == NULL) {
    ...
    fprintf(stdout, "\ntest_membuf: mbuf.position=%zu, mbuf.data=%s\n", membuf_position(&mbuf), data);

    rv = fwrite(data, membuf_position(&mbuf), 1, stdout);
    if (rv < 0) {
